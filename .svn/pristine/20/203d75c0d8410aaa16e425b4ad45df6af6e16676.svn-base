...
Page({
  ...
  xlChange: function (e) {
    this.setData({
      hcType: this.data.list[e.detail.value]
    })
  },
  // 区域
  ...
  formSubmit(e) {
    console.log(e)
    let hcTitle = e.detail.value.title
    let hcIntro = e.detail.value.details
    if (!hcTitle) {
      wx.showToast({
        title: '请输入标题',
        icon: "none"
    ...
    // }
    post({
      link: "/heart/consult/save",
      data: {
        hcTitle: hcTitle,
        hcIntro: hcIntro,
        hcType: this.data.hcType.dictItemCode,
